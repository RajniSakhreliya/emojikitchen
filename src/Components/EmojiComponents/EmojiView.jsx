...
const EmojiView = ({
    ...
    return (
        ...
        <div
            ref={emojiRef}
            className={`${isDisabled ? "cursor-default" : "cursor-pointer"} emojiView transition duration-100 
                ${isDisabled ? "" : isSelected ? "bg-[#E8F0FE]" : "hover:bg-[#F1F3F4]"}`}
            style={styles.view}
            onClick={() => {
        ...
        >
            {isVisible ? (
                !imageError ? (
                    <img
                        loading="lazy"
                        className={`${isDisabled ? 'filter grayscale opacity-20' : 'filter-none opacity-100'}`}
                        src={gStaticUrl ? gStaticUrl : getEmojiFromCode(emoji)}
                        onError={handleImageError}
                        alt="Emoji"
                    />
                ) : (
                    <img className="p-3" src={icError} alt="Error loading emoji" loading="lazy" />
                )
            ) : (
